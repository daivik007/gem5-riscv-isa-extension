...
void _start() {
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < N; j++)
            W[i][j] = (i + j) % 10 + 1;  // fixed: missing semicolon
    }
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < N; j++) {
            ...
            // Fused MAC+ReLU: funct7=0x15, funct3=0x1, opcode=0x2B, rd=a0, rs1=a1, rs2=a2
            // 0010101 01100 01011 001 01010 0101011 = 0x2AC5852B
            asm volatile(".word 0x2AC5852B\n"
                : "+r"(acc) : "r"(a), "r"(b));
        }
        output[i] = (int)acc;
    }
